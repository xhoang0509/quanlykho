...
public class daoXuatKho {
    ...
    public ArrayList<XuatKho> getListXuatKho() {
        ...
        try {
            ...
            while (rs.next()) {
                result.add(new XuatKho(rs.getInt("id_xuat_kho"), rs.getInt("sl_san_pham"),
                        rs.getString("thoi_gian_xuat"), rs.getInt("id_lo_sp"), rs.getInt("id_nv")));
            }
    ...
    public int SoLanXuatKhoTheoThoiGian(String thoi_gian) {
        int so_lan = 0;
        String query = "SELECT COUNT(id_xuat_kho) AS lan_xuat_kho FROM `phieu_xuat_kho` WHERE phieu_xuat_kho.thoi_gian_xuat LIKE '%"
                + thoi_gian + "%'";
        ArrayList<Object> arr = new ArrayList<>();
        try {
            DataProvider.getIntance().open();
    ...
    public boolean KiemTraXuatKho(String id_lo, String slton, String slxuat, String ngay, int id_nv) {
        ...
        String query = "INSERT INTO `phieu_xuat_kho`(`sl_san_pham`, `thoi_gian_xuat`, `id_lo_sp`, `id_nv`) VALUES ("
                + slxuat + ",'" + ngay + "'," + id_lo + "," + id_nv + ")";
        ArrayList<Object> arr = new ArrayList<>();
        DataProvider.getIntance().open();
        ...
        DAO.daoThongBao.getInstance()
                .insertThongBao("[Xuất kho] Nhân viên " + nv.ten_nv + " đã xuất hàng ra kho vào lúc " + ngay, ngay, 2);
    ...
    /*
     * public String[][] FindListXuatKho(ArrayList<XuatKho> DuLieuXuatKho,String
     * ValToSearch)
     * {
     ...
     * {
     * int
     * id_sp=DAO.daoChiTietLoSanPham.getInstance().getChiTietLoSanPham(DuLieuXuatKho
     * .get(i).id_lo).id_sp;
     * String tensp=DAO.daoSanPham.getInstance().getSanPham(id_sp).ten_sp;
     * String loaisp=DAO.daoLoaiSanPham.getInstance().getLoaiSanPham(DAO.daoSanPham.
     * getInstance().getSanPham(id_sp).id_loai_sp).ten_loai_sp;
     * String
     * tennv=DAO.daoTaiKhoan.getInstance().getNhanVien(DuLieuXuatKho.get(i).id_nv).
     * ten_nv;
    ...
    public ArrayList<ThongTinXuat> get20XuatKho(ArrayList<ThongTinXuat> arr, long Trang) {
         ...
         * + "order by `id_xuat_kho` limit "+(trang*20-20)
         * +") as t) order by `id_xuat_kho` limit 20";
         * ArrayList<Object> arr = new ArrayList<>();
         * try{
         * DataProvider.getIntance().open();
         * ResultSet rs = DataProvider.getIntance().excuteQuery(query, arr);
         ...
         * {
         * result.add(new
         * XuatKho(rs.getInt("id_xuat_kho"),rs.getInt("sl_san_pham"),rs.getString(
         * "thoi_gian_xuat"),rs.getInt("id_lo_sp"),rs.getInt("id_nv")));
         * }
         * 
         * DataProvider.getIntance().close();
         * }catch(SQLException ex){
    ...
    public boolean HuyPhieuXuat(int id, int id_nv) {
        ...
        try {
            ...
            DataProvider.getIntance().close();
            JOptionPane.showMessageDialog(null, "Xóa phiếu xuất thành công", "Thông báo",
                    JOptionPane.INFORMATION_MESSAGE);
            DAO.daoThongBao.getInstance().insertThongBao(
                    "[Xuất kho] Nhân viên " + DAO.daoTaiKhoan.getInstance().getNhanVien(id_nv).ten_nv
                            + " đã xóa phiếu xuất vào lúc " + DAO.DateTimeNow.getIntance().Now,
